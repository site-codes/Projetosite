    ...
    <div class="viewmode">
      <div>
        ...
        <main>
            <button
            class="quality 1080p"
            data-source="${link1080p}"
          >
            1080p
          </button>
          <button
            class="quality 720p"
            data-source="${link720p}"
          >
            720p
          </button>
          <button
            class="quality 480p"
            data-source="${link480p}"
          >
            480p
          </button>
        </main>
        ...
        <span data-source="SETA ESQUERDA/ SETA ESQUERDA">Retrocede 10s</span>
...
function updateBrilho() {
    ...
    document.getElementById('percentage').textContent = `${Math.round(currentHeight)}%`;
}
...
blockElement.addEventListener('click', function() {
    if (!blockElement.classList.contains('unlock')) {
        ...
        document.querySelectorAll('.titleepplayer, .logotl, #pular, #leftdd, #rightdd, .medp, .endp, .volubar, .container, .more, #block, .block, .progressline').forEach(function(element) {
...
var elementsToReveal = document.querySelectorAll('.container, .volubar, .topp, .lefttime, .righttime, .endp, .progressline');
