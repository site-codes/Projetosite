...
async function displayAnimes(animes) {
  ...
  for (const [index, anime] of animes.entries()) {
    ...
    <div>
      <button class="epscode" onclick="openEps(${anime.id})">Lista de eps</button>
    </div>
  `;
...
<main class="episodio items ${additionalClass}">
  <div class="vieweP" onclick="playSoundSec()" data-id="${totalEpisodesBeforeSeason + index + 1}">
...
<section class="pageanimeep temporadas">
    <div class="dadospesquisa">
      ...
      <div class="filtrospesq">
         <button id="Olep"></button>
         <button id="Nvep"></button>
...
function openEps(animeId) {
  const modalEpisodes = document.getElementById("episodes");
  modalEpisodes.style.display = "flex"; // Exibe o modal
...
function closeEps() {
  const modalepisodes = document.getElementById("episodes");
  modalepisodes.style.display = "none"; // Fecha o modal  
  document.body.style.overflow = "auto";
}

function loadEpisodes(animeId) {
  const episodeList = document.getElementById("episode-list");
  episodeList.innerHTML = ""; // Limpa a lista existente

  // Buscando detalhes do anime para obter o nome
  fetch(`https://api.themoviedb.org/3/tv/${animeId}?language=pt-BR`, {
    headers: {
      Authorization: `Bearer ${apiToken}`,
      'Content-Type': 'application/json',
    },
  ...
  .then(animeDetails => {
    const animeName = animeDetails.name; // Obtendo o nome do anime
    const totalSeasons = animeDetails.number_of_seasons; 
    const releaseDate = animeDetails.first_air_date;
    const animeEpisodes = animeDetails.number_of_episodes || '';
    const seasonsNumber = animeDetails.seasons.length || '';
    const animeTitle = document.getElementById('animeTitle').value;
    const animeDubbedOrSubbed = document.getElementById('animeDubbedOrSubbed').value;
    const linkPage = document.getElementById('linkPage').value;
  ...
  const FormatedanimeDate = formatDate(releaseDate);
  ...
  const FormatedanimeReleaseDate = formatReleaseDate(releaseDate);
   // formated text title
   function formatAnimeTitle(animeTitle) {
    // Mapeia caracteres acentuados para suas versões sem acento
    const accentsMap = {
  ...
  const animeTitleCustomLInk = formatAnimeTitle(animeTitle);
    ...
    // Agora, busque os episódios para cada temporada
    const seasonPromises = [];
    for (let seasonNumber = 1; seasonNumber <= totalSeasons; seasonNumber++) {
      seasonPromises.push(
        fetch(`https://api.themoviedb.org/3/tv/${animeId}/season/${seasonNumber}?language=pt-BR`, {
          headers: {
            Authorization: `Bearer ${apiToken}`,
            'Content-Type': 'application/json',
          },
    ...
    // Quando todas as promessas de temporada forem resolvidas
    return Promise.all(seasonPromises).then(seasons => ({ seasons, animeName, releaseDate, animeEpisodes, seasonsNumber, FormatedanimeReleaseDate, animeTitleCustomLInk, animeTitle, linkPage, FormatedanimeDate, animeDubbedOrSubbed}));
  })
  .then(({ seasons, animeName, releaseDate,animeEpisodes, seasonsNumber, FormatedanimeReleaseDate, animeTitleCustomLInk, animeTitle, linkPage, FormatedanimeDate, animeDubbedOrSubbed}) => {
// Cria uma div para os inputs logoSite, capaPauseEp e capaPlayEp
const inputGroupDiv = document.createElement("div");
inputGroupDiv.classList.add("input-group");

// Cria um input para logoSite
const inputLogoSite = document.createElement("input");
inputLogoSite.type = "text";
inputLogoSite.placeholder = "Digite a logo do site aqui...";
inputLogoSite.classList.add("episode-input");

// Variável para armazenar o valor da logo do site
let logoSite = ''; 

// Evento para atualizar logoSite
inputLogoSite.addEventListener("input", (event) => {
  logoSite = event.target.value; // Armazena o valor
  // Atualiza todos os episódios com a nova logo
  document.querySelectorAll(".episodeItem .logoSite-display").forEach(display => {
    display.textContent = logoSite; // Atualiza a exibição
  });
});

// Adiciona o input para logoSite na div de inputs
inputGroupDiv.appendChild(inputLogoSite);

// Cria e adiciona o input para capaPlayEp
const inputCapaPlayEp = document.createElement("input");
inputCapaPlayEp.type = "text";
inputCapaPlayEp.placeholder = "Digite a capa Play aqui...";
inputCapaPlayEp.classList.add("episode-input");

let capaPlayEp = 'https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEhLWz9x3ikVPc617QvcWpJcTcBE-CaUbDMKhZ5uuIGgkJwCzkjMNuBluzU_MZLPlRgVZyjb5xnE-8sI4InLjZeciuyB-A49jHunazGC9tsU0_nk4B4upXkfQI07rb0AMamPfYnfwGmDimy6MOfebPMmcElTrmW3kdIxjF3zViP7u9S97qVX2mLZ-WmhiZc/s1600/wepik-modern-simple-anime-festival-halfpage-banner-202407121751473MKs.png'; // Valor padrão para capaPlayEp

inputCapaPlayEp.addEventListener("input", (event) => {
  capaPlayEp = event.target.value; // Armazena o valor
  document.querySelectorAll(".episodeItem .capaPlayEp-display").forEach(display => {
    display.textContent = capaPlayEp; // Atualiza a exibição
  });
});

// Adiciona o input para capaPlayEp na div de inputs
inputGroupDiv.appendChild(inputCapaPlayEp);

// Cria e adiciona o input para capaPauseEp
const inputCapaPauseEp = document.createElement("input");
inputCapaPauseEp.type = "text";
inputCapaPauseEp.placeholder = "Digite a capa Pause aqui...";
inputCapaPauseEp.classList.add("episode-input");

let capaPauseEp = 'https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEjacOot-pxKgbIMj02-ZZfdzO0Gsqdjv9hX6r8PLRXnxQKVTNMsiOYHtVMKp8_n4sYXxlxW2tXPlBbzrWJvtaJ-aP8OSRBbASaiaGC3g9hIWQfV54ApHt607TiMs2CFCTy8meMcHtieVAiX7v8qoMwIM85lLbVEYQlfiU82kbnYxYrdm3oY0QoS8XXmfo8/s1600/1.png'; // Valor padrão para capaPauseEp

inputCapaPauseEp.addEventListener("input", (event) => {
  capaPauseEp = event.target.value; // Armazena o valor
  document.querySelectorAll(".episodeItem .capaPauseEp-display").forEach(display => {
    display.textContent = capaPauseEp; // Atualiza a exibição
  });
});

// Adiciona o input para capaPauseEp na div de inputs
inputGroupDiv.appendChild(inputCapaPauseEp);

// Adiciona a div de inputs na lista de episódios
...
function formatDateToYearMonth(date) {
  ...
  const month = String(d.getMonth() + 1).padStart(2, '0'); // Padroniza o mês para 2 dígitos
  return `/${year}/${month}/`;
}
...
function formatDate(dateString) {
  ...
  return `/${year}/${month}/`; // Retorna a data formatada
}

function addDaysToDate(dateString, daysToAdd) {
  const date = new Date(dateString);
  date.setDate(date.getDate() + daysToAdd);
  return formatDate(date); // Utiliza a função formatDate para retornar a data formatada
}

seasons.forEach((season, seasonIndex) => {
  // Cria uma div para a temporada e adiciona a classe .listaeps
  const seasonContainer = document.createElement("div");
  seasonContainer.classList.add("listaeps"); // Adiciona a classe .listaeps

  const seasonTitle = document.createElement("h3");
  seasonTitle.textContent = `Temporada ${String(seasonIndex + 1).padStart(2, '0')}`;
  seasonContainer.appendChild(seasonTitle); // Adiciona o título à div da temporada
  ...
  season.episodes.forEach((episode, episodeIndex) => {
    ...
    let inicioOP = '00m00s'; // Valor padrão para inicioOP
    let finalOP = '01m30s'; // Valor padrão para finalOP
    let link1080p = ''; // Valor padrão para link 1080p
    let link720p = ''; // Valor padrão para link 720p
    let link480p = ''; // Valor padrão para link 480p
   
    const FormatedanimeReleaseDate = formatDateToYearMonth(episode.air_date); // Formata a data

    // datas
    let nextEpisodeAirDate = "";
    if (episode.air_date) {
      nextEpisodeAirDate = addDaysToDate(episode.air_date, 8);
    }
    const previousEpisodeIndex = episodeIndex - 1;
let previousData = '';
if (season.episodes[previousEpisodeIndex]) {
    previousData = formatDate(season.episodes[previousEpisodeIndex].air_date) || '';
}
    ...
    <span class="episode-cover" >
      ...
      <h8 class="numero">${`Ep${String(episodeIndex + 1).padStart(2, '0')}`}</h8>
      <span>
        <h8>${episode.air_date || 'N/A'}</h8>
        <h8>${animeDubbedOrSubbed}</h8>
      </span>
    </span>
      <div>
        <span class="link">
          ${FormatedanimeReleaseDate}${animeTitleCustomLInk}-t${String(seasonIndex + 1).padStart(2, '0')}-episodio${String(episodeIndex + 1).padStart(2, '0')}-${animeDubbedOrSubbed.toLowerCase()}.html
        </span>
      </div>
`;
// Cria a div para organizar os inputs
const inputContainer = document.createElement("div");
inputContainer.classList.add("input-container");

// Cria e adiciona o input para inicioOP
const inputInicio = document.createElement("input");
inputInicio.type = "text";
inputInicio.placeholder = "Inicio da OP (ex: 00m00s)";
inputInicio.classList.add("episode-input");

inputInicio.addEventListener("input", (event) => {
  inicioOP = event.target.value; // Armazena o valor
  episodeItem.querySelector(".inicioOP-display").textContent = inicioOP; // Atualiza a exibição
});

inputContainer.appendChild(inputInicio); // Adiciona o input à div

// Cria e adiciona o input para finalOP
const inputFinal = document.createElement("input");
inputFinal.type = "text";
inputFinal.placeholder = "Final da OP (ex: 01m30s)";
inputFinal.classList.add("episode-input");

inputFinal.addEventListener("input", (event) => {
  finalOP = event.target.value; // Armazena o valor
  episodeItem.querySelector(".finalOP-display").textContent = finalOP; // Atualiza a exibição
});

inputContainer.appendChild(inputFinal); // Adiciona o input à div

// Cria e adiciona o input para link 1080p
const inputLink1080p = document.createElement("input");
inputLink1080p.type = "text";
inputLink1080p.placeholder = "Link do video 1080p";
inputLink1080p.classList.add("episode-input");

inputLink1080p.addEventListener("input", (event) => {
  link1080p = event.target.value; // Armazena o valor
  episodeItem.querySelector(".link1080p-display").textContent = link1080p; // Atualiza a exibição
});

inputContainer.appendChild(inputLink1080p); // Adiciona o input à div

// Cria e adiciona o input para link 720p
const inputLink720p = document.createElement("input");
inputLink720p.type = "text";
inputLink720p.placeholder = "Link do video 720p";
inputLink720p.classList.add("episode-input");

inputLink720p.addEventListener("input", (event) => {
  link720p = event.target.value; // Armazena o valor
  episodeItem.querySelector(".link720p-display").textContent = link720p; // Atualiza a exibição
});

inputContainer.appendChild(inputLink720p); // Adiciona o input à div

// Cria e adiciona o input para link 480p
const inputLink480p = document.createElement("input");
inputLink480p.type = "text";
inputLink480p.placeholder = "Link do video 480p";
inputLink480p.classList.add("episode-input");

inputLink480p.addEventListener("input", (event) => {
  link480p = event.target.value; // Armazena o valor
  episodeItem.querySelector(".link480p-display").textContent = link480p; // Atualiza a exibição
});

inputContainer.appendChild(inputLink480p); // Adiciona o input à div




// Adiciona a div de inputs ao episodeItem
episodeItem.appendChild(inputContainer);

// Evento para copiar o conteúdo formatado ao clicar no episodeItem
episodeItem.addEventListener("click", (event) => {
  ...
  if (event.target.tagName.toLowerCase() === 'input') {
    return; // Não faz nada se o clique foi em um input
      ...
      // Se não foi um input, copia o texto formatado
...
<div class="wallpaperr">
  ...
  <section id="playerpage">
    <main class="main1">
      <div class="mains">
        ...
        <div class="moreiep">
          ...
          <a id="preview" onclick="playSoundSec()" href="${previousData}${animeTitleCustomLInk}-t${String(seasonIndex + 1).padStart(2, '0')}-episodio${String(episodeIndex).padStart(2, '0')}-${animeDubbedOrSubbed.toLowerCase()}.html"></a>
          <a id="next" onclick="playSoundSec()" href="${nextEpisodeAirDate}${animeTitleCustomLInk}-t${String(seasonIndex + 1).padStart(2, '0')}-episodio${String(episodeIndex + 2).padStart(2, '0')}-${animeDubbedOrSubbed.toLowerCase()}.html"></a>
      ...
      <div class="mainsb">
        ...
        <span class="classesep">
          ...
          <h8>${animeDubbedOrSubbed}</h8>
        <h8 id="datepost">${FormatedanimeDate}</h8>
        </span>
        <div class="buttonslink">
          <a href="${linkPage}.html" onclick="playSoundSec()"></a>
          <a href="${link1080p}" onclick="playSoundSec()"></a>
          <a href="#comment" onclick="playSoundSec()"></a>
        </div>
    ...
    <main class="main2">
      <button id="push" onclick="playSoundSec()"></button>
  ...
  <link rel="stylesheet" href="https://site-codes.github.io/IframePage-EpPlayer/style.css"/>
  <script>
  
    // Inicio e final da Opening
    const inicioOP = "${inicioOP}";
    const finalOP = "${finalOP}";
  
    // Capa banner e para pausa
    const capaPlayEp = "${capaPlayEp}";
    const capaPauseEp = "${capaPauseEp}";
  
    // Titulo
    const TituloEp = "${animeName} - T${String(seasonIndex + 1).padStart(2, '0')}. Episódio ${String(episodeIndex + 1).padStart(2, '0')} - ${episode.name}";
  ...
  TITULO DO EPISODIO:
   ${animeName} - T${String(seasonIndex + 1).padStart(2, '0')}. Episódio ${String(episodeIndex + 1).padStart(2, '0')} - ${animeDubbedOrSubbed} | ${episode.name}

   MARCADORES:
   Episodio, ${animeDubbedOrSubbed}, #${String(episodeIndex + 1).padStart(2, '0')}

   LINK DESSE EP:
   ${FormatedanimeReleaseDate}${animeTitleCustomLInk}-t${String(seasonIndex + 1).padStart(2, '0')}-episodio${String(episodeIndex + 1).padStart(2, '0')}-${animeDubbedOrSubbed.toLowerCase()}.html

   DESCRIÇÃO DE PESQUISA:
   Assistir Temporada ${String(seasonIndex + 1).padStart(2, '0')} episódio ${String(episodeIndex + 1).padStart(2, '0')} de ${animeName} ${animeDubbedOrSubbed} em FULLHD online e gratis 
  ...
  -------------------------------------------------------------------------------------------->
    ...
    // Adiciona o episodeItem ao seasonContainer
    seasonContainer.appendChild(episodeItem);
  });

  // Adiciona a div da temporada ao episódio list
